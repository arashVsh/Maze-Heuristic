...
class PacmanGraphics:
    ...
    def update(self, newState):
        ...
        if newState._foodEaten != None:
            removeFood(newState._foodEaten, self.food)
        if newState._capsuleEaten != None:
            removeCapsule(newState._capsuleEaten, self.capsules)
    ...
    def drawPacman(self, pacman, index):
        ...
        endpoints = self.getEndpoints(getDirection(pacman))
    ...
    def animatePacman(self, pacman, prevPacman, image):
        ...
        if self.frameTime > 0.01 or self.frameTime < 0:
            ...
            for i in range(1, int(frames) + 1):
                ...
                self.movePacman(pos, getDirection(pacman), image)
                refresh()
                sleep(abs(self.frameTime) / frames)
        else:
            self.movePacman(self.getPosition(pacman), getDirection(pacman), image)
        refresh()
    ...
    def moveGhost(self, ghost, ghostIndex, prevGhost, ghostImageParts):
        ...
        self.moveEyes(self.getPosition(ghost), getDirection(ghost), ghostImageParts[-4:])
        refresh()
        ...
        for xNum, x in enumerate(wallMatrix):
            ...
            for yNum, cell in enumerate(x):
                if cell:  # There's a wall here
                    ...
                    wIsWall = isWall(xNum - 1, yNum, wallMatrix)
                    eIsWall = isWall(xNum + 1, yNum, wallMatrix)
                    nIsWall = isWall(xNum, yNum + 1, wallMatrix)
                    sIsWall = isWall(xNum, yNum - 1, wallMatrix)
                    nwIsWall = isWall(xNum - 1, yNum + 1, wallMatrix)
                    swIsWall = isWall(xNum - 1, yNum - 1, wallMatrix)
                    neIsWall = isWall(xNum + 1, yNum + 1, wallMatrix)
                    seIsWall = isWall(xNum + 1, yNum - 1, wallMatrix)
